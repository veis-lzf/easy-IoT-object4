...
wait_event_t client_tts_event = NULL; //是否需要文字转语音
...
int32_t baidu_ai_text2audio(ai_auth_info_t  *auth_info, char *data)  //文字转语音     urlcode转语音       data：传入的是 url
{
    ...
    ret = ai_httpclient_text2audio(auth_info, &httpclient, (char *)ai_host, HTTPS_PORT, (char *)aliyun_iot_ca_get(), HTTPCLIENT_GET, &httpclient_data);   // 文字转语音的请求，响应数据的处理
    return ret;
}
