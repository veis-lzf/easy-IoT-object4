...
void test_thread2(void *pdata)
{
	...
	while(cnt < 10)
	{
		ret = wait_event_timeout(test_event, 2000);
		if(ret < 0)
			p_err("test_event err");
		else if(ret == WAIT_EVENT_TIMEOUT)
			p_dbg("WAIT_EVENT_TIMEOUT");
		else
			p_dbg("线程2被唤醒");
		cnt++;
	}
...
*创建两个线程
*	线程2保持睡眠,等待唤醒信号,2秒超时
*	线程1每秒发送一次唤醒事件给线程2,总共发5次
*
*/
	test_event = init_event(); //创建一个互斥变量
	if(!test_event)
