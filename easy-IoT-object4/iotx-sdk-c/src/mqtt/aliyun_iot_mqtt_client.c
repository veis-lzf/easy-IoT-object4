...
int MQTTSubscribe(MQTTClient_t *c, const char *topicFilter, enum QoS qos, unsigned int msgId, messageHandler messageHandler)
{
    ...
    InitTimer(&timer);
		...
		handler.topicFilter = topicFilter;      //pub消息对应的主题
    handler.fp = messageHandler;    //pub消息处理函数
        ...
        aliot_user_info_pt puser_info)
{
	...
	rc = aliyun_iot_net_init(pClient->ipstack, puser_info->host_name, puser_info->port_str, (char *)puser_info->pubKey);
    if(SUCCESS_RETURN != rc)
    {
        aliyun_iot_mqtt_set_client_state(pClient, CLIENT_STATE_INVALID);
        IOT_FUNC_EXIT_RC(rc);
