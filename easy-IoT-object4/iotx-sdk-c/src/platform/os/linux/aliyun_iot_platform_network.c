...
{
/*
    int32 sockfd = ERROR_NET_UNKNOWN_HOST;
    ...
    if (sockfd < 0) {
		printf("onenet Socket alloc ERROR\n");
        return -1; 
    }
    server = gethostbyname(host);
    if (server == NULL) {
        close(sockfd);
		printf("onenet DNS ERROR\n");
        return -2;
    }
