...
list_t *list_new()
{
    list_t *self = aliyun_iot_memory_malloc(sizeof(list_t));
    if (!self) {
        return NULL;
    }
    self->head = NULL;
...
void list_destroy(list_t *node)
{
    ...
    list_node_t *next;
    list_node_t *curr = node->head;

    while (len--)
    {
        next = curr->next;
        if (node->free) node->free(curr->val);
...
{
    list_iterator_t *self = aliyun_iot_memory_malloc(sizeof(list_iterator_t));
    if (!self)
        return NULL;
    self->next = node;
...
{
    list_node_t *self = aliyun_iot_memory_malloc(sizeof(list_node_t));
    if (!self){
        return NULL;
    }
