...
static u8 io_spi_trans_byte(u8 byte)
{
	uint8_t i; 
	u8 Temp=0x00;
	unsigned char SDI; 
...
//  	NVIC_InitTypeDef   NVIC_InitStructure;
	...
	gpio_cfg((uint32_t)RFID_CLK_GROUP, RFID_CLK_PIN, GPIO_Mode_Out_PP);
	gpio_cfg((uint32_t)RFID_MISO_GROUP, RFID_MISO_PIN, GPIO_Mode_Out_PP);
	gpio_cfg((uint32_t)RFID_MOSI_GROUP, RFID_MOSI_PIN, GPIO_Mode_IPU);
	
	gpio_cfg((uint32_t)RFID_RST_GROUP, RFID_RST_PIN, GPIO_Mode_Out_PP);
	gpio_cfg((uint32_t)RFID_SDA_GROUP, RFID_SDA_PIN, GPIO_Mode_Out_PP);

	gpio_cfg((uint32_t)RFID_IRQ_GROUP, RFID_IRQ_PIN, GPIO_Mode_IPU);

	GPIO_SET(RFID_CLK_GROUP, RFID_CLK_PIN);
	GPIO_SET(RFID_MISO_GROUP, RFID_MISO_PIN);
	GPIO_SET(RFID_MOSI_GROUP, RFID_MOSI_PIN);
	RFID_CS_HIGH;
	RFID_RST_LOW;
