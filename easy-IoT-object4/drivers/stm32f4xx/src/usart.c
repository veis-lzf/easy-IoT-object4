...
void uart1_init(void)
{
	...
	gpio_cfg((uint32_t)UART1_RX_PORT_GROUP, UART1_RX_PIN, GPIO_Mode_AF_IPU);
...
void uart3_init(void)
{
	...
	gpio_cfg((uint32_t)UART3_RX_PORT_GROUP, UART3_RX_PIN, GPIO_Mode_AF_IPU);
...
void select_debug_uart(int sel)
{
	if(sel == 1)
		dbg_uart = USART1;
	else if(sel == 3)
...
int fputc(int ch, FILE *f)
{
	...
	USART3->DR = ch; //再UART3上面也打印调试信息，因跟UART1是同时进行的，所以不必判断其标志位
	USART1->DR = ch;
...
//import from main.c
//extern char command;
extern u8_t command[100];
extern int cmd_index;
void uart1_read_data(u8_t *buff, int size)
{
	//TODO
	command[cmd_index++] = buff[0];
    
       //it's better to delay send event after all data recved.
	if(cmd_index == 1) send_work_event(TEST_EVENT);
}

void send_cmd_to_self(char cmd)
{
       //command = cmd;
       command[cmd_index++] = cmd;
...
void USART3_IRQHandler()
{
	...
	{
		i = USART3->DR;
		uart1_read_data(&i, 1); //uart3也可以发控制命令
	}
	if( sr & (1L<<6))  //txe
	{
