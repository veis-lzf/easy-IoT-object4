...
//  readWriteTiming.FSMC_DataSetupTime = 0X06;			//数据保存时间为60个HCLK	=6*60=360ns
...
//  writeTiming.FSMC_AddressSetupTime =0X10;	      //地址建立时间（ADDSET）为9个HCLK =54ns 
//  writeTiming.FSMC_AddressHoldTime = 0x00;	 //地址保持时间（A		
//  writeTiming.FSMC_DataSetupTime =06;		 //数据保存时间为6ns*9个HCLK=54ns
...
//  writeTiming.FSMC_AccessMode = FSMC_AccessMode_A;	 //模式A 
  ...
  readWriteTiming.FSMC_AddressSetupTime = 0X08;	 //地址建立时间（ADDSET）为16个HCLK 1/168M=6ns*16=96ns	
  readWriteTiming.FSMC_AddressHoldTime = 0x00;	 //地址保持时间（ADDHLD）模式A未用到	
  ...
  readWriteTiming.FSMC_AccessMode = FSMC_AccessMode_A;	 //模式A 
	...
	writeTiming.FSMC_AddressSetupTime =5;	      //地址建立时间（ADDSET）为9个HCLK =54ns 
  writeTiming.FSMC_AddressHoldTime = 0x00;	 //地址保持时间（A		
  writeTiming.FSMC_DataSetupTime = 4;		 //数据保存时间为6ns*9个HCLK=54ns
  writeTiming.FSMC_BusTurnAroundDuration = 0x00;
  writeTiming.FSMC_CLKDivision = 0x00;
  writeTiming.FSMC_DataLatency = 0x00;
  writeTiming.FSMC_AccessMode = FSMC_AccessMode_A;	 //模式A 
...
void DrawRGBImage(uint16_t Xpos, uint16_t Ypos, uint16_t Xsize, uint16_t Ysize, u8 *pData)
{
  ...
  u16 data;
	...
	BlockWrite(Xpos,Xsize+Xpos-1,Ypos,Ysize+Ypos-1);
