...
#else
#define SDIO_TRANSFER_CLK_DIV		    ((uint8_t)0x2)   //45m/(x+2)
...
static void GPIO_Configuration(void)
{
	...
	GPIO_PinAFConfig(SDIO_D0_PORT, SDIO_D0_SOURCE, GPIO_AF_SDIO); 
	GPIO_PinAFConfig(SDIO_D1_PORT, SDIO_D1_SOURCE, GPIO_AF_SDIO); 
#if ENABLE_DATA_WIDE
	GPIO_PinAFConfig(SDIO_D2_PORT, SDIO_D2_SOURCE, GPIO_AF_SDIO); 
	GPIO_PinAFConfig(SDIO_D3_PORT, SDIO_D3_SOURCE, GPIO_AF_SDIO); 
#endif
	GPIO_PinAFConfig(SDIO_CMD_PORT, SDIO_CMD_SOURCE, GPIO_AF_SDIO); 
	GPIO_PinAFConfig(SDIO_CLK_PORT, SDIO_CLK_SOURCE, GPIO_AF_SDIO); 

	/* Configure PC.08, PC.09, PC.10, PC.11, PC.12 pin: D0, D1, D2, D3, CLK pin */
	gpio_cfg((uint32_t)SDIO_D0_PORT, SDIO_D0_PIN , GPIO_Mode_AF_PP);
	gpio_cfg((uint32_t)SDIO_D1_PORT, SDIO_D1_PIN , GPIO_Mode_AF_IPU);
#if ENABLE_DATA_WIDE	
	gpio_cfg((uint32_t)SDIO_D2_PORT, SDIO_D2_PIN , GPIO_Mode_AF_PP);
	gpio_cfg((uint32_t)SDIO_D3_PORT, SDIO_D3_PIN , GPIO_Mode_AF_PP);
#endif	

	gpio_cfg((uint32_t)SDIO_CMD_PORT, SDIO_CMD_PIN, GPIO_Mode_AF_PP);
	gpio_cfg((uint32_t)SDIO_CLK_PORT, SDIO_CLK_PIN, GPIO_Mode_AF_PP);
