 ...
 * @{
 */
#define USB_RE_ACTIVE_TIMEOUT	3
...
#define USB_OTG_MODIFY_REG32(reg,clear_mask,set_mask) do{	\
uint32_t cpu_sr;	\
cpu_sr = local_irq_save();\
USB_OTG_WRITE_REG32(reg, (((USB_OTG_READ_REG32(reg)) & ~clear_mask) | set_mask ) );\
local_irq_restore(cpu_sr);}while(0)

//在中断中执行
//	#define USB_OTG_MODIFY_REG32_INT(reg,clear_mask,set_mask)	
//  USB_OTG_WRITE_REG32(reg, (((USB_OTG_READ_REG32(reg)) & ~clear_mask) | set_mask ) )
