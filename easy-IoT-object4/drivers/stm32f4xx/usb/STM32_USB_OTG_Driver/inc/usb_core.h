...
#define USBH_OTG_ISR_Handler 	OTG_HS_IRQHandler
/*
#define USBH_OTG_ISR_Handler OTG_FS_IRQHandler
 */
...
typedef enum
{
	URB_IDLE = 0, URB_DONE, URB_NOTREADY, URB_ERROR, URB_STALL, URB_NAK
} URB_STATE;
...
typedef struct _HCD
{
	...
	__IO uint8_t ErrCnt[USB_MAX_PIPES];
	__IO uint32_t XferCnt[USB_MAX_PIPES];
	__IO HC_STATUS HC_Status[USB_MAX_PIPES];
	__IO URB_STATE URB_State[USB_MAX_PIPES];
	USB_OTG_HC hc[USB_MAX_PIPES];
	uint16_t channel[USB_MAX_PIPES];
	//       uint8_t re_active_timeout[USB_OTG_MAX_TX_FIFOS];
...
{
	USBH_InterfaceDesc_TypeDef *desc;
...
{
	USB_INTERFACE_UNBOUND = 0, USB_INTERFACE_BINDING, USB_INTERFACE_BOUND, USB_INTERFACE_UNBINDING, 
};
...
{
	USBH_CfgDesc_TypeDef *desc;

	struct usb_interface *interface[USB_MAXINTERFACES];
};

#define INTERFACE_MAX_NUM_ENDPOINTS	3
struct usb_device
{
	//struct net_device *net_dev;

	USBH_DevDesc_TypeDef *descriptor;
	struct usb_host_config *config;

	struct usb_host_endpoint ep[USBH_MAX_NUM_ENDPOINTS];
...
typedef struct USB_OTG_handle
{
	...
	wait_event_t crl_urb_wait;
	wait_event_t bulkout_urb_wait;
	mutex_t ctrl_urb_lock;
	mutex_t bulk_out_urb_lock;
	mutex_t int_out_urb_lock;
	mutex_t iso_out_urb_lock;
	struct urb *urb[USB_MAX_PIPES];
	unsigned char bulk_out_irq_pending;
	unsigned char int_out_irq_pending;
	unsigned char cur_bulk_out_ch;
	timer_t polling_timer;  
