...
URB_STATE HCD_SubmitRequest(struct urb *urb, USB_OTG_CORE_HANDLE *pdev, uint8_t hc_num)
{
	URB_STATE ret;

	pdev->host.URB_State[hc_num] = URB_IDLE;
	pdev->host.hc[hc_num].xfer_count = 0;
	pdev->host.hc[hc_num].packet_count = 0;
	

	pdev->host.hc[hc_num].packet_num = (pdev->host.hc[hc_num].xfer_len + pdev->host.hc[hc_num].max_packet - 1) / pdev->host.hc[hc_num].max_packet;

	if (pdev->host.hc[hc_num].ep_type == USB_OTG_EP_CONTROL)
	{
		clear_wait_event(pdev->crl_urb_wait);
	} 
	else
	{
		pdev->urb[hc_num] = urb;
	...
	if (USB_OTG_OK != USB_OTG_HC_StartXfer(pdev, hc_num))
		p_err("USB_OTG_HC_StartXfer err\n");

	if (pdev->host.hc[hc_num].ep_type == USB_OTG_EP_CONTROL)
	{
		ret = usb_start_wait_urb(pdev, hc_num, 1000);
	}
	else
	{
		ret = URB_DONE;
