...
static uint32_t USB_OTG_HandleOTG_ISR(USB_OTG_CORE_HANDLE *pdev)
{
	...
	if (gotgint.b.hstnegsucstschng)
	{
		...
		/* Host negotiation success                                         */
		{
			...
			/* The core AUTOMATICALLY sets the Host mode
			 */
			{
		...
		/* Host negotiation failure */
		{
			...
			gotgint.b.hstnegsucstschng = 1;
		/* Ack "Host Negotiation Success Status Change" interrupt.          */
	}
...
void USB_OTG_InitiateHNP(USB_OTG_CORE_HANDLE *pdev, uint8_t state, uint8_t mode)
{
	...
	if (mode)
	{
		...
		if (state)
		{
			 ...
			 */
			otgctl.b.hnpreq = 1; /* Initiate an HNP req. to the connected USB
			host*/
			USB_OTG_WRITE_REG32(&pdev->regs.GREGS->GOTGCTL, otgctl.d32);
		}
	}
	else
	{
		...
		if (state)
		{
			...
			hprt0.b.prtsusp = 1; /* The core clear this bit when disconnect
			interrupt generated (GINTSTS.DisconnInt = '1') */
