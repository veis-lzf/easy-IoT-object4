...
static void USBH_ParseCfgDesc(USBH_CfgDesc_TypeDef *cfg_desc, USBH_InterfaceDesc_TypeDef *itf_desc, USBH_EpDesc_TypeDef *ep_desc, uint8_t *buf, uint16_t length);
...
USBH_Status USBH_Get_DevDesc(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost, uint8_t length)
{
	...
	USBH_Status status;
	p_dbg_enter;
	pdev->host.Rx_Buffer = (uint8_t*)mem_malloc(MAX_DATA_LENGTH);
	if (!pdev->host.Rx_Buffer)
		return USBH_FAIL;
	if ((status = USBH_GetDescriptor(pdev, phost, USB_REQ_RECIPIENT_DEVICE | USB_REQ_TYPE_STANDARD, USB_DESC_DEVICE, pdev->host.Rx_Buffer, length)) == USBH_OK)
	{
		/* Commands successfully sent and Response Received */
		//      p_hex("DevDesc", pdev->host.Rx_Buffer, length);
		USBH_ParseDevDesc(&phost->device_prop.Dev_Desc, pdev->host.Rx_Buffer, length);
	}
	mem_free(pdev->host.Rx_Buffer);
	return status;
}
...

{
	...
	if (!pdev->host.Rx_Buffer)
		return USBH_FAIL;
	if ((status = USBH_GetDescriptor(pdev, phost, USB_REQ_RECIPIENT_DEVICE | USB_REQ_TYPE_STANDARD, USB_DESC_CONFIGURATION, pdev->host.Rx_Buffer, length)) == USBH_OK)
	{
		/* Commands successfully sent and Response Received  */
...
USBH_Status USBH_Get_StringDesc(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost, uint8_t string_index, uint8_t *buff, uint16_t length)
{
	...
	if (!pdev->host.Rx_Buffer)
		return USBH_FAIL;
	if ((status = USBH_GetDescriptor(pdev, phost, USB_REQ_RECIPIENT_DEVICE | USB_REQ_TYPE_STANDARD, USB_DESC_STRING | string_index, pdev->host.Rx_Buffer, length)) == USBH_OK)
	{
		/* Commands successfully sent and Response Received  */
...
short USBH_GetStatus(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost, uint8_t ep_num, uint8_t req_type)
{
	short status;
	p_dbg_enter;
	phost->Control.setup.b.bmRequestType = USB_D2H | req_type;
	...
	if (req_type == USB_REQ_RECIPIENT_ENDPOINT)
	{
	...
	USBH_CtlReq(pdev, phost, (uint8_t*) &status, 2);
	p_dbg("status:ep:%d, %x\n", ep_num, status);
	return status;
}
...
USBH_Status USBH_SetFeature(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost, uint8_t ep_num, uint8_t hc_num)
{

	phost->Control.setup.b.bmRequestType =  /*USB_H2D |
	USB_REQ_RECIPIENT_ENDPOINT |
	 	                                               USB_REQ_TYPE_STANDARD*/0x23;

	phost->Control.setup.b.bRequest = USB_REQ_SET_FEATURE;
	phost->Control.setup.b.wValue.w =  /*FEATURE_SELECTOR_ENDPOINT*/0x04;
	phost->Control.setup.b.wIndex.w = ep_num;
	phost->Control.setup.b.wLength.w = 0;
...
USBH_Status USBH_ClrFeature(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost, uint8_t ep_num, uint8_t hc_num)
{
	...
	phost->Control.setup.b.wValue.w =  /*FEATURE_SELECTOR_ENDPOINT*/0x10;
	phost->Control.setup.b.wIndex.w = ep_num;
	phost->Control.setup.b.wLength.w = 0;
                            ...
                            uint8_t hc_num) 
{
  ...
  phost->Control.setup.b.bRequest = USB_REQ_CLEAR_FEATURE;
  phost->Control.setup.b.wValue.w = /*FEATURE_SELECTOR_ENDPOINT*/0;
  phost->Control.setup.b.wIndex.w = ep_num;
  phost->Control.setup.b.wLength.w = 0;           
...
USBH_Status USBH_ClrFeature1(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost, uint8_t ep_num, uint8_t hc_num)
{
	...
	phost->Control.setup.b.bRequest = USB_REQ_CLEAR_FEATURE;
	phost->Control.setup.b.wValue.w =  /*FEATURE_SELECTOR_ENDPOINT*/0x14;
	phost->Control.setup.b.wIndex.w = ep_num;
	phost->Control.setup.b.wLength.w = 0;
...
static void print_devdesc(USBH_DevDesc_TypeDef *dev_desc)
{
	p_dbg("device descriptor");
	p_dbg("	idVendor %d\n", dev_desc->idVendor);
	p_dbg("	idProduct %d\n", dev_desc->idProduct);
	p_dbg("	bDeviceClass %d\n", dev_desc->bDeviceClass);
	p_dbg("	bDeviceSubClass %d\n", dev_desc->bDeviceSubClass);
	p_dbg("	bNumConfigurations %d\n", dev_desc->bNumConfigurations);
}
...
static void USBH_ParseDevDesc(USBH_DevDesc_TypeDef *dev_desc, uint8_t *buf, uint16_t length)
{
	...
	if (length > 8)
	{
		/* For 1st time after device connection, Host may issue only 8 bytes
		for
...
static void print_cfgdesc(USBH_CfgDesc_TypeDef *cfg_desc)
{
	p_dbg("configuration descriptor");
	p_dbg("	bLength %d\n", cfg_desc->bLength);
	p_dbg("	bDescriptorType %d\n", cfg_desc->bDescriptorType);
	p_dbg("	wTotalLength %d\n", cfg_desc->wTotalLength);
	p_dbg("	bNumInterfaces %d\n", cfg_desc->bNumInterfaces);
	p_dbg("	bConfigurationValue %d\n", cfg_desc->bConfigurationValue);
	p_dbg("	iConfiguration %d\n", cfg_desc->iConfiguration);
	p_dbg("	bmAttributes %d\n", cfg_desc->bmAttributes);
	p_dbg("	bMaxPower %d\n", cfg_desc->bMaxPower);
}
 ...
 * @retval None
 */
void print_Interfacedesc(USBH_InterfaceDesc_TypeDef *if_descriptor)
{
	p_dbg("interface descriptor");
	p_dbg("	bLength:%d\n", if_descriptor->bLength);
	p_dbg("	bDescriptorType:%d\n", if_descriptor->bDescriptorType);
	p_dbg("	bInterfaceNumber:%d\n", if_descriptor->bInterfaceNumber);
	p_dbg("	bAlternateSetting:%d\n", if_descriptor->bAlternateSetting);
	p_dbg("	bNumEndpoints:%d\n", if_descriptor->bNumEndpoints);
	p_dbg("	bInterfaceClass:%d\n", if_descriptor->bInterfaceClass);
	p_dbg("	bInterfaceSubClass:%d\n", if_descriptor->bInterfaceSubClass);
	p_dbg("	bInterfaceProtocol:%d\n", if_descriptor->bInterfaceProtocol);
...
static void USBH_ParseInterfaceDesc(USBH_InterfaceDesc_TypeDef *if_descriptor, uint8_t *buf)
{
	...
	print_Interfacedesc(if_descriptor);
...
static void USBH_ParseStringDesc(uint8_t *psrc, uint8_t *pdest, uint16_t length)
{
	...
	if (psrc[1] == USB_DESC_TYPE_STRING)
	{
		...
		psrc += 2; /* Adjust the offset ignoring the String Len and Descriptor
		type */

		for (idx = 0; idx < strlength; idx += 2)
		{
			/* Copy Only the string and ignore the UNICODE ID, hence add the
			src */
			*pdest = psrc[idx];
			pdest++;
		}
