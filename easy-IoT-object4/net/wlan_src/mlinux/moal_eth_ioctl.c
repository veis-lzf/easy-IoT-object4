...
woal_android_priv_cmd(struct net_device *dev, struct ifreq *req)
{
    ...
    ENTER();
    if (copy_from_user(&priv_cmd, req->ifr_ifru.ifru_data, sizeof(android_wifi_priv_cmd))) {
        ret = -EFAULT;
        goto done;
    }
  ...
  handled:
    ...
    if (len > 0) {
        ...
        if (priv_cmd.used_len <= priv_cmd.total_len) {
            ...
            if (copy_to_user
                (req->ifr_ifru.ifru_data, &priv_cmd, sizeof(android_wifi_priv_cmd))) {
                PRINTM(MERROR,
