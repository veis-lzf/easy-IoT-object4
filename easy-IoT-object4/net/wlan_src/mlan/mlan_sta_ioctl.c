...
wlan_set_wpa_ie_helper(mlan_private * priv, t_u8 * ie_data_ptr, t_u16 ie_len)
{
	...
	if (ie_len) {
        ...
        memset(priv->adapter, priv->wpa_ie, 0, priv->max_wpa_ie_len);
        priv->wpa_ie_len = 0;
...
wlan_set_wapi_ie(mlan_private * priv, t_u8 * ie_data_ptr, t_u16 ie_len)
{
    ENTER();
	 ...
	 if (ie_len) {
			...
			memset(priv->adapter, priv->wapi_ie, 0, priv->max_wapi_ie_len);
			priv->wapi_ie_len = (t_u8) ie_len;
        ...
        if (((pvendor_ie->element_id == WPA_IE)
			...
			if (ie_len < (/*sizeof(priv->gen_ie_buf)*/MRVDRV_GENIE_BUF_SIZE - priv->gen_ie_buf_len)) {
				...
				if(priv->max_gen_ie_buf_len < (priv->gen_ie_buf_len + ie_len))
				{
					...
					memcpy(priv->adapter,priv->gen_ie_buf, old_data, priv->gen_ie_buf_len);	
					mem_free(old_data);
