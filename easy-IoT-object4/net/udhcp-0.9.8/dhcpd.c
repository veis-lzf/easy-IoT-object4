...
#define DHCP_RECV_BUFFER_SIZE 512
static char dhcp_buf[DHCP_RECV_BUFFER_SIZE + 4];

int handle_dhcp_rcv(uint8_t *buff, int size);
void dhcpd_recv_thread(void *arg);
...
{
    thread_create(dhcp_server_thread, 0, TASK_DHCP_RECV_PRIO, 0, TASK_DHCP_SERVER_STACK_SIZE, "dhcp_server_thread");
    return 0;
}
...

{
	...
	server_config.start = /*inet_addr("192.168.0.100")*/(get_gw_addr(p_netif) & 0x00ffffff) | (100U << 24);	//地址范围 x.x.x.100 - x.x.x.254
	server_config.end = /*inet_addr("192.168.0.254")*/(get_gw_addr(p_netif) & 0x00ffffff) | (254U << 24);
...
/*
	...
	while(1)
	{
		...
		int size = recvfrom(socket_s, dhcp_buf, DHCP_RECV_BUFFER_SIZE, 0, &remote_addr, (socklen_t*) &retval);

		if(size <= 0)
		{
            p_err("dhcp recv error");
			sleep(10);
			//close(socket_s);    //Now we dont handle this error
		}
		
        p_err("dhcp recv len = %d", size);
		retval = handle_dhcp_rcv((uint8_t*)dhcp_buf, size);
		if(retval < 0)
		{
            p_err("dhcp handle error = %d", retval);
			sleep(10);
		}
	}
	return 0;
