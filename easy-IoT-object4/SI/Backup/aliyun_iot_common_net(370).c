...
{     p_dbg_enter;
    ...
    do
    {
        ...
        if (ret < 0) {
            ...
            aliyun_iot_network_settimeout(pNetwork->handle, 5000, IOT_NET_TRANS_RECV);
...
{     p_dbg_enter;
    ...
    do {
        ...
        if (ret < 0) {
            ...
            aliyun_iot_network_settimeout(pNetwork->handle, 5000, IOT_NET_TRANS_SEND);
...
{    p_dbg_enter;
//    printf("\r\n******aliyun_iot_net_connect*******\r\n");
    if (NULL == pNetwork->ca_crt) { //TCP connection    TCP Á¬½ÓÍøÂç
        return connect_tcp(pNetwork);
    } else { //SSL connection
        return connect_ssl(pNetwork);    //SSL connection  //SSL Á¬½ÓÍøÂç
    }
}


int aliyun_iot_net_init(pNetwork_t pNetwork, char *host, char *port, char *ca_crt)                        //×¢²áÍøÂçº¯Êý
{
//    printf("\r\n******aliyun_iot_net_init*******\r\n");
     p_dbg_enter;
    ...
    pNetwork->handle = -1;
    pNetwork->read = aliyun_iot_net_read;                                                                  //×¢²á¶Áº¯Êý
    pNetwork->write= aliyun_iot_net_write;                                                                 //×¢²áÐ´º¯Êý
    pNetwork->disconnect = aliyun_iot_net_disconnect;                                                     //×¢²á¶Ï¿ªº¯Êý
    pNetwork->connect = aliyun_iot_net_connect;                                                           //×¢²áÁ¬½Óº¯Êý
